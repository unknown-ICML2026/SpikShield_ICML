...
def get_data_paths(args):
    ...
    if data_type == "bcr":
        return {
            "base": base,
            "train": os.path.join(base, "bcr_train_cls9.xlsx"),
...
def get_checkpoint_path(args):
    ...
    return os.path.join(base, "snn_ensemble_checkpoint")

def get_args():
    parser = argparse.ArgumentParser(description="settings for SNN Ensemble Model")

    parser.add_argument("--data_type", type=str, choices=["bcr","mnist","ecg"], default="bcr", help="Data type: bcr or mnist or ecg")
    parser.add_argument("--input_size", type=int, default=None, help="input_size")
    parser.add_argument("--mode", type=str, choices=["train","eval"], default="eval", help="train or eval")
    ...
    parser.add_argument("--beta", type=float, default=0.9, help="leaky beta")
...
def setup_args(args):
    ...
    if args.data_type == "bcr":
        ...
        args.classes = len(train_ds_tmp.targetMap)
        assert train_ds_tmp[0][0].numel() == args.input_size, (
            f"ECG input_size mismatch: expected {args.input_size}, "
            f"got {train_ds_tmp[0][0].numel()}"
        )
    return args
...
class SNNEnsemble(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_steps, beta):
        super().__init__()
        self.num_steps = num_steps
        self.fc_in = nn.Linear(input_dim, hidden_dim)
        self.lif_in = snn.Leaky(beta=beta, spike_grad=spike_grad)
        self.fc_mid = nn.Linear(hidden_dim, output_dim)
        self.lif_mid = snn.Leaky(beta=beta)
    ...
    def forward(self, x):
        ...
        for step in range(self.num_steps):
            spk, mem_in = self.lif_in(x, mem_in)
            if step == 0:
                spk_rec_in = spk
            else:
                spk_rec_in = spk_rec_in + spk

        mem_mid = self.lif_mid.init_leaky()
        mid = self.fc_mid(spk_rec_in)
        for step in range(self.num_steps):
            spk, mem_mid = self.lif_mid(mid, mem_mid)
            if step == 0:
                spk_rec_mid = spk
            else:
                spk_rec_mid = spk_rec_mid + spk

        out = self.fc_out(spk_rec_mid)
        return out
...
def compute_accuracy(model, dataloader):
    ...
    with torch.no_grad():
        for xb, yb in dataloader:
            ...
            total += yb_cuda.size(0)
            correct += (predicted == yb_cuda).sum().item()
...
def train(args):
    ...
    for fold, (train_ids, valid_ids) in enumerate(kfold.split(train_val_indices, y_all)):
        ...
        valid_ids_subsampler  = torch.utils.data.SubsetRandomSampler(valid_ids)
                    ...
                    if epoch % save_interval == 0:
                        if val_pred_acc > best_val_acc[fold]:
                            ...
                            if (max(best_val_acc)==best_val_acc[fold]):
                                best_model_path = (
                                    f"{checkpoint_path}_{args.classes}_{args.num_steps}_{args.hidden_size}_{epoch}_FIN.pt"
                                )
        ...
        log_line = (f"Fold {fold+1} | Best Epoch: {best_epoch} | "
    ...
    log_line2 = (f"Global Best Epoch: {global_best_epoch} | "
...
def load_target_model(args):
    ...
    pattern_fin = re.compile(
        rf"{re.escape(checkpoint_prefix)}_{args.classes}_{args.num_steps}_{args.hidden_size}_(\d+)_FIN\.pt"
    )
    pattern = re.compile(
        rf"{re.escape(checkpoint_prefix)}_{args.classes}_{args.num_steps}_{args.hidden_size}_(\d+)\.pt"
    )
    latest_epoch = -1
    latest_path = None
    ...
    if latest_path is None:
        print("[CKPT] No FIN checkpoint found.")
        for fname in os.listdir(checkpoint_dir):
